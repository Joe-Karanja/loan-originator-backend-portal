...
@Component({
  selector: 'app-list-unit-trust-applications',
  templateUrl: './list-unit-trust-applications.component.html',
  styleUrls: ['./list-unit-trust-applications.component.scss'],
  providers: [DatePipe, CurrencyPipe]
})
export class ListUnitTrustApplicationsComponent implements OnInit {
    ...
    this._httpService.post('', model).subscribe(
      result => {
        this.dataSet = result.list.filter((rec) => {
          return Number(rec.product_id) == 2;
        });
      },
      error => {
      },
