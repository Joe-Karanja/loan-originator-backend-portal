...
@Component({
  selector: 'app-list-institutional-investments',
  templateUrl: './list-institutional-investments.component.html',
  styleUrls: ['./list-institutional-investments.component.scss'],
  providers: [DatePipe, CurrencyPipe]
})
export class ListInstitutionalInvestmentsComponent implements OnInit {
    ...
    this._httpService.post('', model).subscribe(
      result => {
        this.dataSet = result.list.filter((rec) => {
          return Number(rec.product_id) == 4;
        });
      },
      error => {
      },
