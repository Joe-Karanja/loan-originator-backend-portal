...
export class CompletedPaymentsComponent implements OnInit {
  ...
  constructor(config: NgbModalConfig, private _httpService: HttpService, private modalService: NgbModal, private router: Router,
    ...
    this.settings = {
      ...
      actions: {
        ...
        delete: false,
        custom: [
         { name: 'viewRecord', title: '<i class="fa text-warning fa-envelope"> Policy</i> </br>'  },
         { name: 'viewRecord', title: '<i class="fa  text-danger fa-envelope"> Sticker</i>' }
        ],
  ...
  private loadData(): any {
    ...
    this._httpService.post('', model).subscribe(
      result => {
        this.dataSet = result.list.payment_completed;
      },
      error => {
      },
      complete => {
  ...
  private viewRecord(data: any) {
   //  this.router.navigate(['/motor/payments', data.applied_product_id]);
  }
